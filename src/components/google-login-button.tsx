// "use client"

// import { useState } from "react"
// import { Loader2 } from "lucide-react"

// export function GoogleLoginButton() {
//   const [isLoading, setIsLoading] = useState(false)

//   // Generate a random state value for CSRF protection
//   const generateState = () => {
//     return Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15)
//   }

//   const handleGoogleLogin = () => {
//     setIsLoading(true)

//     // Generate and store state in sessionStorage for verification later
//     const state = generateState()
//     sessionStorage.setItem("oauth_state", state)

//     // Construct Google OAuth URL
//     const googleClientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID
//     const redirectUri = `${window.location.origin}/api/auth/callback/google`

//     const googleAuthUrl = new URL("https://accounts.google.com/o/oauth2/v2/auth")
//     googleAuthUrl.searchParams.append("client_id", googleClientId || "")
//     googleAuthUrl.searchParams.append("redirect_uri", redirectUri)
//     googleAuthUrl.searchParams.append("response_type", "code")
//     googleAuthUrl.searchParams.append("scope", "openid email profile")
//     googleAuthUrl.searchParams.append("state", state)

//     // Redirect to Google OAuth
//     window.location.href = googleAuthUrl.toString()
//   }

//   return (
//     <button
//       onClick={handleGoogleLogin}
//       disabled={isLoading}
//       className="w-full bg-gray-200 text-gray-800 px-6 py-3 rounded-md font-medium hover:bg-gray-300 transition-colors flex items-center justify-center"
//     >
//       {isLoading ? (
//         <>
//           <Loader2 className="animate-spin mr-2 h-4 w-4" />
//           Connecting...
//         </>
//       ) : (
//         <>
//           <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
//             <path
//               d="M22.56 12.25c0-.78-.07-1.53-.2-2.23H12v4.51h5.92c-.26 1.34-1.04 2.48-2.21 3.25v2.69h3.57c2.08-1.92 3.28-4.74 3.28-8.22z"
//               fill="#4285F4"
//             />
//             <path
//               d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
//               fill="#34A853"
//             />
//             <path
//               d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
//               fill="#FBBC05"
//             />
//             <path
//               d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
//               fill="#EA4335"
//             />
//           </svg>
//           Sign in with Google
//         </>
//       )}
//     </button>
//   )
// }

































"use client"

import { useState, useEffect, useCallback, useRef } from "react"
import { Loader2 } from "lucide-react"

declare global {
  interface Window {
    google?: {
      accounts: {
        id: {
          initialize: (config: any) => void
          renderButton: (element: HTMLElement, config: any) => void
          prompt: () => void
        }
      }
    }
  }
}

export function GoogleLoginButton() {
  const [isLoading, setIsLoading] = useState(true)
  const [buttonRendered, setButtonRendered] = useState(false)
  const [fallbackVisible, setFallbackVisible] = useState(false)
  const buttonRef = useRef<HTMLDivElement>(null)

  const handleCredentialResponse = useCallback((response: any) => {
    console.log("Google credential response received:", response)

    // Store the credential in sessionStorage
    sessionStorage.setItem("google_credential", response.credential)

    // Decode the JWT to get user info
    try {
      const base64Url = response.credential.split(".")[1]
      const base64 = base64Url.replace(/-/g, "+").replace(/_/g, "/")
      const jsonPayload = decodeURIComponent(
        atob(base64)
          .split("")
          .map((c) => "%" + ("00" + c.charCodeAt(0).toString(16)).slice(-2))
          .join(""),
      )

      const user = JSON.parse(jsonPayload)
      console.log("Decoded user info:", user)

      // Store user info in sessionStorage
      sessionStorage.setItem(
        "user",
        JSON.stringify({
          id: user.sub,
          name: user.name,
          email: user.email,
          picture: user.picture,
        }),
      )

      // Redirect to home page
      window.location.href = "/"
    } catch (error) {
      console.error("Error decoding JWT:", error)
    }
  }, [])

  // Create a direct Google OAuth URL
  const googleOAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${
    process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID
  }&redirect_uri=${encodeURIComponent(
    typeof window !== "undefined" ? `${window.location.origin}/auth/callback` : "",
  )}&response_type=token&scope=email%20profile&prompt=select_account`

  useEffect(() => {
    // Show fallback button after 5 seconds if Google button doesn't render
    const fallbackTimer = setTimeout(() => {
      if (!buttonRendered) {
        console.log("Fallback timer triggered - showing fallback button")
        setFallbackVisible(true)
      }
    }, 5000)

    // Function to initialize Google Sign-In
    const initializeGoogleSignIn = () => {
      if (!window.google || !buttonRef.current) {
        console.log("Google not available or button ref not ready, retrying...")
        return false
      }

      try {
        console.log("Initializing Google Identity Services with client ID:", process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID)
        window.google.accounts.id.initialize({
          client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "",
          callback: handleCredentialResponse,
          auto_select: false,
        })

        console.log("Button ref exists:", !!buttonRef.current)
        window.google.accounts.id.renderButton(buttonRef.current, {
          theme: "outline",
          size: "large",
          width: buttonRef.current.offsetWidth,
          text: "signin_with",
          shape: "rectangular",
        })

        setButtonRendered(true)
        console.log("Google button rendered successfully")
        return true
      } catch (error) {
        console.error("Error initializing Google Identity Services:", error)
        setFallbackVisible(true)
        return false
      }
    }

    // Load the Google Identity Services script
    const loadGoogleScript = () => {
      console.log("Loading Google Identity Services script")
      const script = document.createElement("script")
      script.src = "https://accounts.google.com/gsi/client"
      script.async = true
      script.defer = true

      script.onload = () => {
        console.log("Google Identity Services script loaded")
        setIsLoading(false)

        // Try to initialize immediately
        if (!initializeGoogleSignIn()) {
          // If initialization fails, retry after a short delay
          console.log("Initial initialization failed, retrying after delay")
          setTimeout(() => {
            if (!buttonRendered) {
              initializeGoogleSignIn()
            }
          }, 1000)
        }
      }

      script.onerror = (error) => {
        console.error("Error loading Google Identity Services script:", error)
        setIsLoading(false)
        setFallbackVisible(true)
      }

      document.head.appendChild(script)
    }

    // Start loading the script
    loadGoogleScript()

    return () => {
      // Clean up
      clearTimeout(fallbackTimer)
      const existingScript = document.querySelector('script[src="https://accounts.google.com/gsi/client"]')
      if (existingScript && existingScript.parentNode) {
        existingScript.parentNode.removeChild(existingScript)
      }
    }
  }, [handleCredentialResponse, buttonRendered])

  return (
    <div className="w-full">
      {isLoading ? (
        <button
          disabled
          className="w-full bg-gray-200 text-gray-800 px-6 py-3 rounded-md font-medium flex items-center justify-center"
        >
          <Loader2 className="animate-spin mr-2 h-4 w-4" />
          Loading Google Sign-In...
        </button>
      ) : (
        <>
          {/* Always render the button div but hide it if not needed */}
          <div
            ref={buttonRef}
            className="w-full h-12"
            style={{ display: buttonRendered && !fallbackVisible ? "block" : "none" }}
          ></div>

          {/* Fallback button */}
          {fallbackVisible && (
            <a
              href={googleOAuthUrl}
              className="w-full bg-white border border-gray-300 text-gray-800 px-6 py-3 rounded-md font-medium hover:bg-gray-50 transition-colors flex items-center justify-center"
              onClick={() => {
                console.log("Fallback Google sign-in button clicked")
                console.log("Redirecting to:", googleOAuthUrl)
              }}
            >
              <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.23H12v4.51h5.92c-.26 1.34-1.04 2.48-2.21 3.25v2.69h3.57c2.08-1.92 3.28-4.74 3.28-8.22z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Sign in with Google
            </a>
          )}
        </>
      )}
    </div>
  )
}
